...
def analyze_with_deepl():
    ...
    try:
        ...
        try:
            ...
            logger.error(f"DeepL客户端初始化失败: {str(e)}", exc_info=True)
        ...
        try:
            # 批量翻译到中文
            logger.info(f"开始翻译{len(keywords_to_translate)}个关键词到中文")
            text_to_translate_zh = '\n'.join(keywords_to_translate)
            result_zh = deepl_client.translate_text(text_to_translate_zh, target_lang="ZH")
            chinese_translations = result_zh.text.split('\n')

            # 批量翻译到英文
            logger.info(f"开始翻译{len(keywords_to_translate)}个关键词到英文")
            text_to_translate_en = '\n'.join(keywords_to_translate)
            result_en = deepl_client.translate_text(text_to_translate_en, target_lang="EN-US")
            english_translations = result_en.text.split('\n')
            ...
            while len(english_translations) < len(keywords_to_translate):
                english_translations.append(keywords_to_translate[len(english_translations)])
        ...
        for i, (keyword, count) in enumerate(top_keywords):
            percentage = round((count / total_count) * 100, 2) if total_count > 0 else 0
                      # 英文翻译
            english_word = english_translations[i] if i < len(english_translations) else keyword        
            # 中文翻译
